...
export default function EditTaskModal({ isOpen, onClose, task }: Props) {
  ...
  const [category, setCategory] = useState(task?.category || "water");
  ...
  useEffect(() => {
    if (task) {
      setDate(task.date || "");
      setCategory(task.category || "water");
      setPlantingId(task.plantingId || null);

      
      setDosage(task?.payload?.dosage ? task.payload.dosage.replace(/[^\d.]/g, '') : "");
      setDilution(task.payload?.dilution || "");
      setType(task.payload?.type || "");
      setNote(task.payload?.note || "");
    }
  }, [task]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      alert("Error al actualizar la tarea: " + (error as Error).message);
    }
  };
